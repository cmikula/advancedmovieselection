...
class FileInfo(object):
    ...
    def _getHeader(self):
        ...
        data.append(paddedString(str(self.modificationTime), MOD_TIME_BUFFER_LENGTH))
        data.append(paddedString(str(self.userId), ID_BUFFER_LENGTH))
        data.append(paddedString(str(self.groupId), ID_BUFFER_LENGTH))
        data.append(paddedString("%o" % self.fileMode, FILE_MODE_BUFFER_LENGTH))
    ...
    #fromFilePath = staticmethod(_fromFilePath, doc="Create a FileInfo instance from a file.")
...
class ArFile(object):
    ...
    def __init__(self):
        """
        """
        self.files = [] # TODO: Append FileInfo objects instead?
    ...
    def packed(self):
        ...
        for theFilePath in self.files:
            content.append(FileInfo.fromFilePath(theFilePath).packed())
        return "".join(content)
